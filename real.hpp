...
namespace mpfr {
  ...
  struct type_traits<real<_prec, _rnd>, mpfr_t, _overwrite> {
    ...
    inline static int add(mpfr_ptr rop, mpfr_srcptr op1, mpfr_srcptr op2, mpfr_rnd_t rnd) {
      ...
      //      casting to float
      float a, b, c;
      a = mpfr_get_flt(op1, rnd);
      b = mpfr_get_flt(op2, rnd);
      c = a + b;
      return MPFR_NS mpfr_set_flt(rop, c, rnd);
#else
#pragma message("real.hpp:1158 : compiled")
    ...
    inline static int sub_b(mpfr_ptr rop, mpfr_srcptr op1, mpfr_srcptr op2, mpfr_rnd_t rnd) {
      ...
      return MPFR_NS mpfr_set_flt(rop, c, rnd);
#else
#pragma message("real.hpp:1180 : compiled")
      return MPFR_NS mpfr_sub(rop, op1, op2, rnd);
#endif
    }
...
namespace mpfr {
  ...
  operator +(const _Tp1& r1, const _Tp2& r2) {
    ...
    //      fprintf(stderr, "%s %d\n", __FILE__, __LINE__);
      float rr1, rr2, rr;
      rr1 = mpfr_get_flt(temp1._x, rnd);
      rr2 = mpfr_get_flt(temp2._x, rnd);
      rr = rr1 + rr2;
      mpfr_set_flt(temp._x, rr, rnd);
#else
  ...
  class real {
	...
	//	fprintf(stderr, "%s %d\n", __FILE__, __LINE__);
	float rr1, rr;
	rr1 = mpfr_get_flt(temp._x, _rnd);
	rr = mpfr_get_flt(_x, _rnd);
	rr += rr1;
	mpfr_set_flt(_x, rr, _rnd);	
#else		
...
#pragma message("real.hpp:3887 : operator duoble()")
        return MPFR_NS mpfr_get_d(_x, _rnd);
      }
    //     explicit operator float() const {    
      inline operator float() const {
#pragma message("real.hpp:3892 : operator float()")	
