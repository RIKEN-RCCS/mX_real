...
namespace mX_real {
  namespace tX_real {
	...
	return y;
      ...
      //
      //
      INLINE constexpr tx_real() NOEXCEPT {
        for(auto i=0; i<L; i++) { x[i] = fp<T>::zero(); }
      }
      INLINE constexpr tx_real( T const& x0, T const& x1, T const& x2 ) NOEXCEPT {
	...
	scaling(*this);	
      ...
      INLINE constexpr tx_real( T const &h ) NOEXCEPT {
...
//	scaling(*this);		
//      }
      INLINE constexpr tx_real( T const *d ) NOEXCEPT {
#if MX_REAL_USE_INF_NAN_EXCEPTION
	...
	scaling(*this);
      ...
      INLINE constexpr tx_real( _TX_<T,_A_> const& h ) NOEXCEPT {
        using TX = _TX_<T,_A_>;
#if MX_REAL_USE_INF_NAN_EXCEPTION
	...
	return ldexp(s, iexp);
      ...
      //
      static INLINE TX_REAL<> constexpr zero () NOEXCEPT {
        return TX_REAL<>{ fp<T>::zero() };
      }
      static INLINE TX_REAL<> constexpr one () NOEXCEPT {
        return TX_REAL<>{ fp<T>::one() };
      }
      static INLINE TX_REAL<> constexpr two () NOEXCEPT {
        return TX_REAL<>{ fp<T>::two() };
      }
      static INLINE TX_REAL<> constexpr nhalf () NOEXCEPT {
        return TX_REAL<>{ fp<T>::nhalf() };
      }
      static INLINE TX_REAL<> constexpr nan () NOEXCEPT {
        TX_REAL<> c;
        T p = fp<T>::nan();
        for(auto i=0; i<L; i++) { c.x[i] = p; }
        return c;
      }
      static INLINE TX_REAL<> constexpr inf () NOEXCEPT {
        TX_REAL<> c;
        T p = fp<T>::inf();
        for(auto i=0; i<L; i++) { c.x[i] = p; }
        return c;
      ...
      //
      static INLINE TX_REAL<> constexpr abs ( DX_REAL<> const& a ) NOEXCEPT;
      static INLINE TX_REAL<> constexpr abs ( TX_REAL<> const& a ) NOEXCEPT;
      static INLINE TX_REAL<> constexpr abs ( QX_REAL<> const& a ) NOEXCEPT;
      static INLINE TX_REAL<> constexpr sqrt ( DX_REAL<> const& a ) NOEXCEPT;
      static INLINE TX_REAL<> constexpr sqrt ( TX_REAL<> const& a ) NOEXCEPT;
      static INLINE TX_REAL<> constexpr sqrt ( QX_REAL<> const& a ) NOEXCEPT;
      static INLINE TX_REAL<> constexpr fabs ( DX_REAL<> const& a ) NOEXCEPT {
        return TX_REAL<>::abs(a);
      }
      static INLINE TX_REAL<> constexpr fabs ( TX_REAL<> const& a ) NOEXCEPT {
        return TX_REAL<>::abs(a);
      }
      static INLINE TX_REAL<> constexpr fabs ( QX_REAL<> const& a ) NOEXCEPT {
        return TX_REAL<>::abs(a);
      }
    ...
    INLINE auto constexpr operator== ( tX_real::tx_real<T,A> const& a, Ts const& b ) NOEXCEPT {
      return tX_real::operator_eq ( a, tX_real::tx_real<T,A>(b) );
    }

        template < typename T, Algorithm A, typename Ts, T_scalar(Ts), T_neq_Ts(T,Ts) >
    INLINE auto constexpr operator== ( Ts const& a, tX_real::tx_real<T,A> const& b  ) NOEXCEPT {
      return tX_real::operator_eq ( tX_real::tx_real<T,A>(a) , b);
    }
    ...
    template < typename T, Algorithm A, typename Ts, T_scalar(Ts), T_neq_Ts(T,Ts) >
    INLINE auto constexpr operator> ( tX_real::tx_real<T,A> const& a, Ts const& b ) NOEXCEPT {
      return tX_real::operator_gt ( a, tX_real::tx_real<T,A>(b) );
    }
    template < typename T, Algorithm A, typename Ts, T_scalar(Ts), T_neq_Ts(T,Ts) >
    INLINE auto constexpr operator> ( Ts const& a, tX_real::tx_real<T,A> const& b ) NOEXCEPT {
      return tX_real::operator_gt ( tX_real::tx_real<T,A>(a), b );
    }
    ...
    template < typename T, Algorithm A, typename Ts, T_scalar(Ts), T_neq_Ts(T,Ts) >
    INLINE auto constexpr operator< ( tX_real::tx_real<T,A> const& a, Ts const& b ) NOEXCEPT {
      return tX_real::operator_lt ( a, tX_real::tx_real<T,A>(b) );
    }
    template < typename T, Algorithm A, typename Ts, T_scalar(Ts), T_neq_Ts(T,Ts) >
    INLINE auto constexpr operator< ( Ts const& a, tX_real::tx_real<T,A> const& b ) NOEXCEPT {
      return tX_real::operator_lt ( tX_real::tx_real<T,A>(a), b );
    }
    ...
    INLINE auto constexpr reversed_sign ( tX_real::tx_real<T,Aa> const& a ) NOEXCEPT {
      ...
      tX_real::tx_real<T,Aa> b( -a.x[0], -a.x[1], -a.x[2]);
      b.iexp = a.iexp;
      return b;      
    }
    //
	...
	rescaling(aa, (ia - ib));
	if ( A <= Algorithm::WeakAccurate ) {	  	  	
	  QxW::add_TW_TW_TW ( _TX_(aa), _TX_(b), _TX_(c) );
	} else {
	  QxW::add_QTW_QTW_QTW ( _TX_(aa), _TX_(b), _TX_(c) );
	  if ( A != Algorithm::Quasi ) { Normalize( c ); }
	}
	...
	rescaling(bb, (ib - ia));
	if ( A <= Algorithm::WeakAccurate ) {	  	  	
	  QxW::add_TW_TW_TW ( _TX_(a), _TX_(bb), _TX_(c) );
	} else {	  
	  QxW::add_QTW_QTW_QTW ( _TX_(a), _TX_(bb), _TX_(c) );
	}
	if ( A != Algorithm::Quasi ) { Normalize( c ); }
	...
	rescaling(aa, (ia - ib));
	if ( A <= Algorithm::WeakAccurate ) {	  	  	
	  QxW::add_TW_DW_TW ( _TX_(aa), _DX_(b), _TX_(c) );
	} else {
	  QxW::add_QTW_PA_QTW ( _TX_(aa), _DX_(b), _TX_(c) );
	  if ( A != Algorithm::Quasi ) { Normalize( c ); }
	}
	scaling(c, ib); // c.iexp = 0
      }
	...
	rescaling(bb, (ib - ia));
	if ( A <= Algorithm::WeakAccurate ) {	  	  	
	  QxW::add_TW_DW_TW ( _TX_(a), _DX_(bb), _TX_(c) );
	} else {	
	  QxW::add_QTW_PA_QTW ( _TX_(a), _DX_(bb), _TX_(c) );
	  if ( A != Algorithm::Quasi ) { Normalize( c ); }
	}
	scaling(c, ia); // c.iexp = 0
      } else {
 ...
 // T-S-T
    ...
    INLINE auto constexpr operator_add_exception ( T const& a, TXb const& b, bool & flag ) NOEXCEPT {
      return tX_real::operator_add_exception ( b, a, flag );
    }
#endif
    //
	...
	rescaling(bb, (ib - ia));
	if ( A <= Algorithm::WeakAccurate ) {	    
	  QxW::add_TW_TW_TW ( _TX_(a), _TX_(bb), _TX_(a) );
	} else {	  
	  QxW::add_QTW_QTW_QTW ( _TX_(a), _TX_(bb), _TX_(a) );
	  if ( A != Algorithm::Quasi ) { Normalize( a ); }
	}
	scaling(a, ia);
      }
	...
	rescaling(bb, (ib - ia));	
	if ( A <= Algorithm::WeakAccurate ) {
	  QxW::add_TW_DW_TW ( _TX_(a), _DX_(bb), _TX_(a) );
	} else {
	  QxW::add_QTW_PA_QTW ( _TX_(a), _DX_(bb), _TX_(a) );
	  if ( A != Algorithm::Quasi ) { Normalize( a ); }
	}
	scaling(a, ia);	
      }
    ...
    template < typename T, Algorithm Aa, Algorithm Ab >
    INLINE auto constexpr operator_add_ow_body ( tX_real::tx_real<T,Aa> & a, sX_real::sx_real<T,Ab> const& b ) NOEXCEPT {
      if ( Aa <= Algorithm::WeakAccurate ) {
	...
	rescaling(bb, (ib - ia));
	if ( A <= Algorithm::WeakAccurate ) {	  
	  QxW::add_DW_DW_DW ( _DX_(a), _DX_(bb), _DX_(a) );
	} else {	  
	  QxW::add_PA_PA_PA ( _DX_(a), _DX_(bb), _DX_(a) );
	  if ( A != Algorithm::Quasi ) { Normalize( a ); }
	}
	scaling(a, ia);
      }
  ...
  //  
    template < typename T, Algorithm Aa >
    INLINE auto constexpr operator_add_ow_body ( dX_real::dx_real<T,Aa> & a, sX_real::sx_real<T,Aa> const& b ) NOEXCEPT {
      if ( Aa <= Algorithm::WeakAccurate ) {
    ...
    INLINE auto constexpr operator+= ( dX_real::dx_real<T,A> & a, Ts const& b ) NOEXCEPT {
      return dX_real::operator_add_ow ( a, tX_real::tx_real<T,A>(b) );
    }
    //
    ...
    template < typename T, Algorithm Aa, Algorithm Ab>
    INLINE auto constexpr operator_mul_body ( qX_real::qx_real<T,Aa> const& a, sX_real::sx_real<T,Aa> const& b ) NOEXCEPT {
      using TX = tX_real::tx_real<T,Aa>;
      TX c;
      if ( Aa <= Algorithm::WeakAccurate ) {
    ...
    INLINE auto constexpr operator_mul_exception ( T const& a, TXb const& b, bool & flag ) NOEXCEPT {
      return tX_real::operator_mul_exception ( b, a, flag );
    }
#endif
    //
    template < typename TXa, typename TXb, T_mX2(TXa,TXb), T_assert( TXa::L >= TXb::L ) >
...
#if MX_REAL_USE_INF_NAN_EXCEPTION
      auto flag = false;
      auto e = tX_real::operator_mul_exception ( sa, b, flag );
      if ( flag ) { return e; }
#endif
      auto c = b;
      for(auto i=0; i<3; i++) {
    ...
    template < typename T, Algorithm Aa, Algorithm Ab  >
    INLINE auto constexpr operator_mul_ow_body ( tX_real::tx_real<T,Aa> & a, sX_real::sx_real<T,Ab> const& b ) NOEXCEPT {
      if ( Aa <= Algorithm::WeakAccurate ) {
    ...
    void  madd_( tX_real::tx_real<T,A> const & a, tX_real::tx_real<T,Ab> const& b, tX_real::tx_real<T,A> & c) NOEXCEPT {
      using TX = tX_real::tx_real<T,A>;
      TX ab;
      ...
      clear_underflow<T>();
#endif
      QxW::mul_QTW_QTW_QTW ( _TX_(a), _TX_(b), _TX_(ab) );
      if (ia + ib < ic) {
	rescaling(ab, ((ia + ib) - ic));
        QxW::add_QTW_QTW_QTW(_TX_(c), _TX_(ab), _TX_(c));
	if ( A != Algorithm::Quasi ) { Normalize( c ); }
	scaling(c, ic);        	
      }
      else if (ia + ib > ic) {
    ...
    // T-D-T        
    template < typename T, Algorithm A, Algorithm Ab>
    void  madd_( tX_real::tx_real<T,A> const & a, dX_real::dx_real<T,Ab> const& b, tX_real::tx_real<T,A> & c) NOEXCEPT {
      //      fprintf(stderr, "%s %d : QxW::madd_PA_QTW_QTW \n", __FILE__, __LINE__);
      using TX = tX_real::tx_real<T,A>;
      TX ab;
      ...
      clear_underflow<T>();
#endif
      QxW::mul_QTW_PA_QTW ( _TX_(a), _DX_(b), _TX_(ab) );
      if (ia + ib < ic) {
	rescaling(ab, (ia + ib) - ic);
        QxW::add_QTW_QTW_QTW(_TX_(c), _TX_(ab), _TX_(c));
	if ( A != Algorithm::Quasi ) { Normalize( c ); }
    ...
    //
    template < typename T, Algorithm A, Algorithm Ab>
    void  madd_( dX_real::dx_real<T,Ab> const& a, tX_real::tx_real<T,A> const & b, tX_real::tx_real<T,A> & c) NOEXCEPT {
      madd_(b, a, c);
    }
    //
    // Division
    ...
    //
    template < typename T, Algorithm A >
    INLINE auto constexpr operator_div_pow2 ( tX_real::tx_real<T,A> const& a, T const& b ) NOEXCEPT {
      auto sb = fp<T>::one() / b;
#if MX_REAL_USE_INF_NAN_EXCEPTION
      auto flag = false;
      auto e = tX_real::operator_div_exception ( a, sb, flag );
      if ( flag ) { return e; }
#endif
      auto c = a;
      for(auto i=0; i<3; i++) {
    ...
    template < typename T, Algorithm Aa, Algorithm Ab  >
    INLINE auto constexpr operator_div_ow_body ( tX_real::tx_real<T,Aa> & a, sX_real::sx_real<T,Ab> const& b ) NOEXCEPT {
      if ( Aa <= Algorithm::WeakAccurate ) {
    ...
    //
    template < typename T, Algorithm A >
    INLINE tX_real::tx_real<T,A> constexpr tX_real::tx_real<T,A>::abs ( dX_real::dx_real<T,A> const& a ) NOEXCEPT {
      return tX_real::operator_abs ( a );
    }
    template < typename T, Algorithm A >
    ...
    template < typename T, Algorithm A >
    INLINE tX_real::tx_real<T,A> constexpr tX_real::tx_real<T,A>::abs ( qX_real::qx_real<T,A> const& a ) NOEXCEPT {
      return tX_real::operator_abs ( a );
    }
    //
	...
	if ( A <= Algorithm::WeakAccurate ) {
	  QxW::sqrt_TW_TW ( _TX_(aa), _TX_(c) );
	} else {
	  QxW::sqrt_QTW_QTW ( _TX_(aa), _TX_(c) );
	  if ( A != Algorithm::Quasi ) { Normalize( c ); }
	}
      }
      scaling(c, ia);
	...
	if ( A <= Algorithm::WeakAccurate ) {
	  QxW::sqrt_DW_TW ( _DX_(aa), _TX_(c) );
	} else {
	  QxW::sqrt_PA_QTW ( _DX_(aa), _TX_(c) );
	  if ( A != Algorithm::Quasi ) { Normalize( c ); }
	}
      }
      scaling(c, ia);
    ...
    INLINE auto constexpr operator_sqrt_exception ( T const& a, bool & flag ) NOEXCEPT {
      using TX = tX_real::tx_real<T,A>;
      {
        if ( fp<T>::isnan( a ) ) { flag = true; auto c=a; return TX{ c,c,c }; }
        if ( fp<T>::is_negative( a ) ) { flag = true; return TX::nan(); }
        if ( fp<T>::isinf( a ) ) { flag = true; return TX::inf(); }
        if ( fp<T>::is_zero( a ) ) { flag = true; auto c=a; return TX{ c,c,c }; }
      }
      //
      flag = false; return TX::zero();
    }
    ...
    INLINE auto constexpr operator_sqrt ( TXa const& a ) NOEXCEPT {
      ...
      if ( a.is_negative() ) { return tX_real::mX_real<TXa>::nan(); };
      if ( a.isinf() ) { return tX_real::mX_real<TXa>::inf(); }
#endif
      Algorithm constexpr A = TXa::base_A;
      if ( A != Algorithm::Quasi ) {
    ...
    //
    template < typename T, Algorithm A >
    INLINE tX_real::tx_real<T,A> constexpr tX_real::tx_real<T,A>::sqrt ( dX_real::dx_real<T,A> const& a ) NOEXCEPT {
      return tX_real::operator_sqrt ( a );
    }
    template < typename T, Algorithm A >
    ...
    template < typename T, Algorithm A >
    INLINE tX_real::tx_real<T,A> constexpr tX_real::tx_real<T,A>::sqrt ( qX_real::qx_real<T,A> const& a ) NOEXCEPT {
      return tX_real::operator_sqrt ( a );
    }
    //
