...
def gen_comparison_op( description, op, not_op, func, Tc ) :
    ...
    print( '    INLINE auto constexpr operator{op} ( {m}X_real::{m}x_real<T,Aa> const& a, {m}X_real::{m}x_real<T,Ab> const& b ) NOEXCEPT {{'.format( m=mX_type(Tc), op=op ) )
    ...
    print( '    }' )
    print( '    template < typename T, Algorithm Aa, Algorithm Ab >' )
    print( '    INLINE auto constexpr operator{not_op} ( {m}X_real::{m}x_real<T,Aa> const& a, {m}X_real::{m}x_real<T,Ab> const& b ) NOEXCEPT {{'.format( not_op=not_op, m=mX_type(Tc) ) )
